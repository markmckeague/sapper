...
function select_route(url) {
    ...
    if (routes.ignore.some(function (pattern) { return pattern.test(path); }))
        return;
    var _loop_1 = function (i) {
        var page = routes.pages[i];
        var match = page.pattern.exec(path);
        if (match) {
            var params = page.params(match);
            var query_1 = {};
            ...
            return { value: { url: url, route: page, props: { params: params, query: query_1, path: path } } };
        }
    };
    for (var i = 0; i < routes.pages.length; i += 1) {
...
function prepare_route(Page, props) {
    ...
    return Promise.resolve(Page.preload.call({
        ...
        if (error) {
            return error_route().then(function (_a) {
                var Page = _a["default"];
...
function goto(href, opts) {
    ...
    if (target) {
        ...
        promise = new Promise(function (f) { }); // never resolves
    }
    return promise;
}
function prefetchRoutes(pathnames) {
    ...
    return routes.pages
        ...
        if (!pathnames)
            return true;
        return pathnames.some(function (pathname) { return route.pattern.test(pathname); });
    })
