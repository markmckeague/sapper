...
function middleware(_a) {
    var App = _a.App, routes = _a.routes, store = _a.store;
    if (!App) {
    ...
    var output = __chunk_2.locations.dest();
...
function serve(_a) {
    ...
    return function (req, res, next) {
        if (filter(req)) {
            var type = lookup(req.path);
            try {
                var data = read(req.path.slice(1));
...
function get_server_route_handler(routes) {
    function handle_route(route, req, res, next) {
        ...
        var handle_method = route.handlers[method_export];
        if (handle_method) {
...
function get_page_handler(App, routes, store_getter) {
    var output = __chunk_2.locations.dest();
    var get_chunks = __chunk_2.dev()
        ? function () { return JSON.parse(fs.readFileSync(path.join(output, 'client_assets.json'), 'utf-8')); }
        : (function (assets) { return function () { return assets; }; })(JSON.parse(fs.readFileSync(path.join(output, 'client_assets.json'), 'utf-8')));
    var template = __chunk_2.dev()
        ? function () { return fs.readFileSync(__chunk_2.locations.app() + "/template.html", 'utf-8'); }
        : (function (str) { return function () { return str; }; })(fs.readFileSync(__chunk_2.locations.dest() + "/template.html", 'utf-8'));
    var server_routes = routes.server_routes, pages = routes.pages;
    var error_route = routes.error;
    function handle_route(route, req, res, status, error) {
        ...
        var link = []
            ...
            .map(function (file) { return "<" + req.baseUrl + "/client/" + file + ">;rel=\"preload\";as=\"script\""; })
            .join(', ');
        res.setHeader('Link', link);
        var store = store_getter ? store_getter(req) : null;
        var props = { params: req.params, query: req.query, path: req.path };
        if (route.error) {
        ...
        var redirect;
        var preload_error;
        Promise.resolve(route.handler.preload ? route.handler.preload.call({
            ...
            fetch: function (url$$1, opts) {
                var parsed = new url.URL(url$$1, "http://127.0.0.1:" + process.env.PORT + (req.baseUrl ? req.baseUrl + '/' : ''));
                if (opts) {
                    opts = Object.assign({}, opts);
                    var include_cookies = (opts.credentials === 'include' ||
                        opts.credentials === 'same-origin' && parsed.origin === "http://127.0.0.1:" + process.env.PORT);
                    if (include_cookies) {
                        if (!opts.headers)
                            opts.headers = {};
                        var str = []
                            .concat(cookie.parse(req.headers.cookie || ''), cookie.parse(opts.headers.cookie || ''), cookie.parse(res.getHeader('Set-Cookie') || ''))
                            .map(function (cookie$$1) {
                            return Object.keys(cookie$$1)
                                .map(function (name) { return name + "=" + encodeURIComponent(cookie$$1[name]); })
                                .join('; ');
                        })
                            .filter(Boolean)
                            .join(', ');
            ...
            var _a = App.render({ Page: route.handler, props: props }, {
                store: store
            }), html = _a.html, head = _a.head, css = _a.css;
            var scripts = []
                .concat(chunks.main) // chunks main might be an array. it might not! thanks, webpack
                .filter(function (file) { return !file.match(/\.map$/); })
                .map(function (file) { return "<script src='" + req.baseUrl + "/client/" + file + "'></script>"; })
                .join('');
            var inline_script = "__SAPPER__={" + [
                "baseUrl: \"" + req.baseUrl + "\"",
                serialized.preloaded && "preloaded: " + serialized.preloaded,
                serialized.store && "store: " + serialized.store
            ].filter(Boolean).join(',') + "};";
            var has_service_worker = fs.existsSync(path.join(__chunk_2.locations.dest(), 'service-worker.js'));
            ...
            var page = template()
                ...
                .replace('%sapper.scripts%', function () { return "<script>" + inline_script + "</script>" + scripts; })
                .replace('%sapper.html%', function () { return html; })
                .replace('%sapper.head%', function () { return "<noscript id='sapper-head-start'></noscript>" + head + "<noscript id='sapper-head-end'></noscript>"; })
                .replace('%sapper.styles%', function () { return (css && css.code ? "<style>" + css.code + "</style>" : ''); });
    ...
    return function find_route(req, res) {
        if (!server_routes.some(function (route) { return route.pattern.test(req.path); })) {
